...
COORD coord;
int coin = 50, cnt = 0, cost = 0,miss = 0;
int rt[3] = { 0 };
...
void result() {
	...
	if (rt[0] == -1) {
		...
		if (rt[1] == -1) {
			coin += 3;
			if (rt[2] == -1) {
				coin += 10; 	printf("小当たり：チェリー     ");miss=0;
				}
		}
	}

	//その他 全部同じ もしくは １０回以上外れかつ左２つ同じ または30回以上外れ 
